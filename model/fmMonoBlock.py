...
from fmSupportLib import fmDemodArctan#,\
     #self_demod,filterCoeff,own_block_proc_func
...
if __name__ == "__main__":
	...
	#-------------------------------------------------------------------------
	#derive filter coeff using own coeff function for front-end LPF
	#rf_coeff = filterCoeff(rf_Fs,rf_Fc,rf_taps)
	...
	#-------------------------------------------------------------------------
	#derive filter coeff using own coeff function for extracted audio
	#audio_coeff = filterCoeff(rf_Fs/rf_decim,audio_Fc,audio_taps)
	...
	audio_save = np.zeros(audio_taps-1)
	state_phase = 0
	...
	while (block_count+1)*block_size < len(iq_data):
		...
		#-------------------------------------------------------------------------
		#perform block processing use own function
		#i_filt,state_i_lpf_100k=own_block_proc_func(\
        #           iq_data[(block_count)*block_size:(block_count+1)*block_size:2],\
        #            rf_coeff,state_i_lpf_100k)
		#q_filt,state_q_lpf_100k=own_block_proc_func(\
        #            iq_data[(block_count)*block_size+1:(block_count+1)*block_size:2],\
		...
		#-------------------------------------------------------------------------
        #demod with own written function
		#fm_demod, prev_I,prev_Q = self_demod(i_ds, q_ds,prev_I,prev_Q,block_count)
        
        #-------------------------------------------------------------------------
        
		# extract the mono audio data through filtering
		audio_filt,audio_save=signal.lfilter(audio_coeff, 1.0, fm_demod,zi=audio_save)
		
		
		#-------------------------------------------------------------------------
		#perform block processing use own function
		#audio_filt,audio_save=own_block_proc_func(fm_demod,audio_coeff,audio_save)
		...
		if block_count >= 10 and block_count < 12:
			...
			# save figure to file
			fig.savefig("../data/fmMonoBlock_own_func" + str(block_count) + ".png")
